...
describe("xpath", () => {
	...
	describe(".select()", () => {
		...
		it("should select with namespaces, using a resolver", () => {
			...
			assert.strictEqual(
				"JKR",
				xpath.selectWithResolver('//testns:field[@testns:type="author"]/text()', doc, resolver)[0]
					.nodeValue
			);
		...
		it("should select from xml with a default namespace, using a resolver", () => {
			...
			assert.strictEqual(
				"Harry Potter",
				xpath.selectWithResolver("//testns:title/text()", doc, resolver)[0].nodeValue
			);
			assert.strictEqual(
				"JKR",
				xpath.selectWithResolver('//testns:field[@type="author"]/text()', doc, resolver)[0]
					.nodeValue
			);
		});
		...
		it("should select with namespaces, prefixes different in xml and xpath, using a resolver", () => {
			...
			assert.strictEqual(
				"JKR",
				xpath.selectWithResolver('//ns:field[@ns:type="author"]/text()', doc, resolver)[0].nodeValue
			);
		});
		...
		it("should select with namespaces, using namespace mappings", () => {
			...
			assert.strictEqual(
				"JKR",
				select('//testns:field[@testns:type="author"]/text()', doc)[0].nodeValue
			);
		});
	...
	describe("selection", () => {
		...
		// https://github.com/NikhilVerma/xpath-next/issues/37
		it("should select multiple attributes", () => {
			...
			// https://github.com/NikhilVerma/xpath-next/issues/41
		...
		it("should evaluate local-name() and name() on processing instructions", () => {
			var xml =
				'<?book-record added="2015-01-16" author="J.K. Rowling" ?><book>Harry Potter</book>';
			var doc = new dom().parseFromString(xml);
			...
			assert.deepEqual(
				expectedName,
				localName,
				'local-name() - "' + expectedName + '" !== "' + localName + '"'
			);
		...
		it("should compare nodesets to nodesets (=)", () => {
			...
			var doc = new dom().parseFromString(xml);
			var houses = xpath
				.parse("/school/houses/house[student = /school/honorStudents/student]")
				.select({ node: doc });

			assert.strictEqual(houses.length, 2);

			var houseNames = houses
				.map(function (node) {
		...
		it("should support various inequality expressions on nodesets", () => {
			...
			var lhsPaths = ["/books/book[@num <= $theNumber]", "/books/book[@num < $theNumber]"];
	...
	describe("type conversion", () => {
		...
		it("should convert numbers to strings correctly", () => {
			assert.strictEqual(
				...
				xpath.parse("0.525 div 1000000 div 1000000 div 1000000 div 1000000").evaluateString()
			);
			assert.strictEqual(
				"525000000000000000000000",
				xpath.parse("0.525 * 1000000 * 1000000 * 1000000 * 1000000").evaluateString()
			);
		});
	...
	describe("parsed expressions", () => {
		it("should work with no options", () => {
			...
			assert.strictEqual(
				typeof parsed.evaluate,
				"function",
				"parsed.evaluate should be a function"
			);
			assert.strictEqual(
				typeof parsed.evaluateNumber,
				"function",
				"parsed.evaluateNumber should be a function"
			);
		...
		it("should support .evaluateString() and .evaluateNumber()", () => {
			...
			assert.strictEqual(
				typeof parsed.evaluateString,
				"function",
				"parsed.evaluateString should be a function"
			);
			assert.strictEqual("7", parsed.evaluateString({ node: doc }));

			assert.strictEqual(
				typeof parsed.evaluateBoolean,
				"function",
				"parsed.evaluateBoolean should be a function"
			);
		...
		it("should support .evaluateBoolean()", () => {
			...
			assert.strictEqual(
				false,
				evaluate("/*/myrtle"),
				"boolean value of empty node set should be false"
			);
			...
			assert.strictEqual(
				true,
				evaluate("/*/title"),
				"boolean value of non-empty nodeset should be true"
			);
			...
			assert.strictEqual(
				false,
				evaluate('/*/title != "Harry Potter"'),
				"title != Harry Potter should be false"
			);

			assert.strictEqual(
				false,
				evaluate('/*/title = "Percy Jackson"'),
				"title should not equal Percy Jackson"
			);
		});
		...
		it("should support custom functions in namespaces", () => {
			...
			var parsed = xpath.parse(
				'concat(hp:double(/*/title), " is 2 cool ", hp:square(2), " school")'
			);
			...
			assert.strictEqual(
				"Harry PotterHarry Potter is 2 cool 4 school",
				parsed.evaluateString(context)
			);
		...
		it("should support .toString()", () => {
			...
			assert.strictEqual(
				withPredicates.toString(),
				"child::book[(child::number > 3)][contains(child::title, 'and the')]"
			);

			var parenthesisWithPredicate = parser.parse("(/books/book/chapter)[7]");

			assert.strictEqual(
				parenthesisWithPredicate.toString(),
				"(/child::books/child::book/child::chapter)[7]"
			);

			var charactersOver20 = parser.parse("heroes[age > 20] | villains[age > 20]");

			assert.strictEqual(
				charactersOver20.toString(),
	...
	describe("html-mode support", () => {
		...
		it("should support the isHtml option", () => {
			...
			var greetings2 = xpath
				.parse("/h:html/h:bOdY/h:p")
				.select({ node: docHtml, namespaces: ns, isHtml: true });
	...
	describe("functions", () => {
		...
		// https://github.com/NikhilVerma/xpath-next/issues/32
